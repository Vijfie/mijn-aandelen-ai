...
class MassBacktester {
  ...
  async runMassBacktests() {
    console.log("Ì¥¨ Starting MASS backtesting campaign...");
    ...
    console.log(`ÌæØ Running ${scenarios.length} different backtest scenarios...");
    console.log("‚è∞ This will take several minutes due to AI API calls...");
    console.log("");
    
    for (let i = 0; i < scenarios.length; i++) {
      const scenario = scenarios[i];
      console.log(`\\n[${i+1}/${scenarios.length}] Ì¥¨ Testing: ${scenario.name}`);
      console.log(`   Ì≥ä Symbols: ${scenario.symbols.join(", ")}`);
      console.log(`   Ì≥Ö Period: ${scenario.startDate} to ${scenario.endDate}`);
  ...
  async generateMassReport() {
    ...
    console.log("Ì≥ä AGGREGATE STATISTICS:");
    ...
    console.log(`   ‚è∞ Total Duration: ${this.results.reduce((sum, r) => sum + parseFloat(r.duration), 0).toFixed(1)}s`);
    
    // Best strategy analysis
    const best = sorted[0];
    console.log("\\nÌæØ BEST STRATEGY ANALYSIS:");
    console.log(`   ÌøÜ Winner: ${best.scenario}`);
    console.log(`   Ì≥à Return: ${best.totalReturn.toFixed(2)}%`);
    console.log(`   Ì≥ä Risk-Adjusted: ${(best.sharpeRatio || 0).toFixed(2)} Sharpe`);
    console.log(`   Ì≤º Efficiency: ${(best.totalReturn / best.totalTrades).toFixed(2)}% per trade`);
    
    console.log("\\n‚ú® Mass backtesting completed!");
    console.log("Ì≤æ All results saved to database for analysis");
